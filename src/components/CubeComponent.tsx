...
const FaceComponent: React.FC<FaceComponentProps> = ({ condition: { face, stickers, size } }) => {
    const cells = [];
    for (let row = 0; row < size; row++) {
        for (let col = 0; col < size; col++) {
            cells.push(<div key={`pif-${row}-${col}`} className={`cell pif-top-${row} pif-left-${col} ${consts.colorMap[stickers[row * consts.size + col]]}`}></div>);
        }
    }
